...
import sys
sys.path.append("/content/drive/MyDrive/MultiModalClassifier-main")
...
#os.environ['TORCH_HOME'] = '/content/drive/MyDrive/MultiModalClassifier-main' #setting the environment variable

CHECKPOINT_PATH="/content/drive/MyDrive/models/checkpoint"
...
#    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    img_width=300
    img_height=300
    data_path='/content/drive/MyDrive/hw2/custom/train'
    batch_size=64

    #Load dataset
    #dataloaders, dataset_sizes, class_names, img_shape = loadTorchdataset("custom",'trainonly',data_path, img_height, img_width, batch_size)
    #Visualize dataset
    ...
    train_data = datasets.ImageFolder(data_path, transform=auto_transforms)
    ...
    dataset_sizes = {'train': len(train_set), 'val': len(
        val_set)}
    ...
    summary(model=model_ft,
            input_size=(batch_size, 3, img_height, img_width), # make sure this is "input_size", not "input_shape"
    ...
    lr_scheduler = setupLearningratescheduler('StepLR', optimizer_ft, epoch, STEPS_PER_EPOCH)
    ...
    model_ft = train_model(model_ft, dataloaders, dataset_sizes, criterion, optimizer_ft, lr_scheduler,
                       start_epoch=0, num_epochs=epoch,
                       tensorboard_writer=tensorboard_writer, profile=prof, checkpoint_path=save_path)

    #save torch model
    # modelsavepath = os.path.join(args.save_path, 'model_best.pt')
    # torch.save(model_ft.state_dict(), modelsavepath)

    #test_model(model_ft, dataloaders, class_names, criterion, batch_size, key='val')
...
#    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    #test_model(model_ft, dataloaders, class_names, criterion, 32, key='val')
